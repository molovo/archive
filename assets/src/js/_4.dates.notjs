...
( function () {
  ...
  var Dates = {
    ...
    focusAction: function ( evt, self ) {
      ...
      // reset the enteredNumber string
      self.enteredNumber = "";
    },
    ...
    enterNumber: function ( act, pos, k, self ) {
      ...
      clearTimeout( self.finishedTyping );
      ...
      self.finishedTyping = setTimeout( function () {
        self.enteredNumber = "";
      }, 5000 );
    },
    ...
    removeNumber: function ( act, pos, k, self ) {
      // If we're not running as a result of a backspace, escape
      if ( k !== 8 ) return false;

      // Check the position, update accordingly
      if ( pos >= 0 && pos < 3 ) {
        self.enteredNumber = "dd";
        self.setDay( act, pos, k, self );
      } else if ( pos >= 3 && pos < 6 ) {
        self.enteredNumber = "mm";
        self.setMonth( act, pos, k, self );
      } else if ( pos >= 6 ) {
        self.enteredNumber = "yyyy";
        self.setYear( act, pos, k, self );
      }
    ...
    setYear: function ( act, pos, k, self ) {
      ...
      dd[ 2 ] = num.substring( num.length - 4, num.length );
    ...
    setupValues: function ( fields ) {
      ...
      for ( i = fields.length - 1; i >= 0; i-- ) {
        ...
        // Strip any extra characters (needed to remove timestamps from the end of date values)
        act.value = act.value.substring( 0, 10 );
    ...
    isValue: function ( value ) {
      var reg = /(0[1-9]|[12][0-9]|3[01]|dd)[\/](0[1-9]|1[012]|dd)[\/]((19|20)\d\d|dd)/;
      return value.match( reg );
    },
